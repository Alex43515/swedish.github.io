...
class Price_List extends Module_Base {
	...
	protected function _register_controls() {
		...
		$this->add_control(
			'item_counter',
			[
				'label'        => __( 'Item Counter', 'bdthemes-element-pack' ),
        ...
        $this->add_control(
			'counter_heading',
			[
				'label'     => __( 'Counter', 'bdthemes-element-pack' ),
				'type'      => Controls_Manager::HEADING,
				'separator' => 'before',
				'condition' => [
					'item_counter' => 'yes',
		...
		$this->add_group_control(
			...
			[
				...
				'selector'  => '{{WRAPPER}} .bdt-price-list-item::before',
				'condition' => [
					'item_counter' => 'yes',
				],
				'exclude' => [
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .bdt-price-list-item::before' => 'color: {{VALUE}}',
				],
				'condition' => [
					'item_counter' => 'yes',
				]
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .bdt-price-list-item' => 'padding-left: {{SIZE}}{{UNIT}};',
				],
				'condition' => [
					'item_counter' => 'yes',
				]
	...
	private function render_item_header( $item ) {
		...
		$bdt_has_counter = ( 'yes' === $settings['item_counter'] ) ? ' bdt-price-list-counter' : '';
        ...
        if ( $url ) {
			...
			return '<li class="bdt-price-list-item'. esc_attr( $bdt_has_counter ) .'"><div '. $this->get_render_attribute_string( $unique_link_id ) .'bdt-grid>';
		} else {
			return '<li class="bdt-price-list-item bdt-grid bdt-grid-small bdt-flex-'. esc_attr( $settings['vertical_align'] . $bdt_has_image . $bdt_has_counter ) .'" bdt-grid>';
		}
	}
	...
	protected function _content_template() {
		...
		<ul class="bdt-price-list">
			<#
			var bdt_has_counter = ( 'yes' === settings.item_counter ) ? ' bdt-price-list-counter' : '';

				for ( var i in settings.price_list ) {
					var item = settings.price_list[i],
						item_open_wrap = '<li class="bdt-price-list-item bdt-grid bdt-grid-small bdt-flex-'+ settings.vertical_align + bdt_has_counter +'" bdt-grid>',
						item_close_wrap = '</li>';
					if ( item.link.url ) {
						item_open_wrap = '<li class="bdt-price-list-item'+ bdt_has_counter +'"><div class="bdt-grid bdt-grid-small bdt-flex-' + settings.vertical_align + '" href="' + item.link.url + '" bdt-grid>';
