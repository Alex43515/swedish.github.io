...
class WPUF_Pro_Subscription {
    ...
    public function __construct() {
        ...
        //Rollback counter for CPT
        add_action( 'wp_trash_post', [ $this, 'restore_post_numbers' ] );
    ...
    public function wpuf_send_subs_notification() {
        ...
        foreach ( $users as $user ) {
            ...
            if ( ! empty( $sub ) && ( $sub !== 'Cancel' || $sub !== 'cancel' ) && $enabled_sub_noti === 'on' ) {
                if ( isset( $sub['recurring'] ) && $sub['recurring'] === 'yes' ) {
                    return;
                }

                $exp_time  = date_create( $sub['expire'] );
                $curr_time = date_create( gmdate( 'Y-m-d h:i:s', time() ) );
                $time_diff = date_diff( $curr_time, $exp_time );

                if ( ( $curr_time < $exp_time ) && ( $time_diff->days <= $date_before ) ) {
                    $this->wpuf_pre_sub_exp_notification( $user, $sub );
                } elseif ( ( $curr_time > $exp_time ) && ( $time_diff->days >= $date_after ) ) {
    ...
    public function prepare_mail_body( $user, $sub_pack, $mail_body ) {
        ...
        $duration      = $sub_meta['_billing_cycle_number'];
        $duration_type = $sub_meta['_cycle_period'];
        $start_date    = $wpdb->get_var( $wpdb->prepare( "SELECT created FROM {$wpdb->prefix}wpuf_transaction WHERE pack_id !=0 AND user_id=%s ORDER BY id DESC LIMIT 1 ", $user->ID ) );

        if ( $start_date ) {
            $start_date = gmdate( 'l\, F Y', strtotime( $start_date ) );
            $end_date   = gmdate( 'l\, F Y', strtotime( $start_date . " + {$duration} {$duration_type}" ) );
        }
