...
class WPUF_Frontend_Form_Profile extends WPUF_Frontend_Render_Form {
    ...
    public function user_register() {
        ...
        $form_settings         = $form->get_settings();
        $form_fields           = $form->get_fields();
        $attachments_to_delete = isset( $_POST['delete_attachments'] ) ? sanitize_text_field( wp_unslash( $_POST['delete_attachments'] ) ) : array();
        ...
        if ( $pass_element ) {
            ...
            $password        = isset( $_POST['pass1'] ) ? sanitize_text_field( wp_unslash( $_POST['pass1'] ) ) : false;
     ...
     * @return void
     */
    public function send_user_notification( $user_id, $user_email, $notification_type = null, $form_id ) {
    ...
    public function send_welcome_email( $user_id, $form_id ) {
        ...
        $body = sprintf(
            __( "Hi %1\$username%,\r\n\r\nCongrats! You are Successfully registered to %2\$s.\r\n\r\nThank you very much.", 'wpuf-pro' ),
            $blogname
        );
