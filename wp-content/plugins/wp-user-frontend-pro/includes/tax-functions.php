...
function wpuf_base_country_state( $args ) {
    ...
    <table style="width:80%; table-layout:auto;" id="wpuf-base-country-state" class="wp-list-table">
        ...
        <tr>
            ...
            $selected['country'] = ! empty( $selected['country'] ) ? $selected['country'] : 'US';
            ?>
...
function wpuf_save_tax_options() {
    if ( isset( $_REQUEST['option_page'] ) && $_REQUEST['option_page'] === 'wpuf_payment_tax' ) {
        ...
        $base_state = isset( $_REQUEST['wpuf_base'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['wpuf_base'] ) ) : '';
        $tax_rates  = isset( $_REQUEST['wpuf_tax_rates'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['wpuf_tax_rates'] ) ) : '';
