...
/**
  ...
  Plugin URI: https://wedevs.com/docs/wp-user-frontend-pro/modules/user-listing-profile/
...
class WPUF_User_Listing {
    ...
    public function enqueue_scripts() {
        wp_enqueue_style( 'wpuf-user-listing', plugins_url( 'css/profile-listing.css', __FILE__ ), [], WPUF_PRO_VERSION );
    }
    ...
    public function admin_menu_top() {
        ...
        $profile_builder_page = add_submenu_page( 'wp-user-frontend', __( 'Profile Builder', 'wpuf-pro' ), __( 'Profile Builder', 'wpuf-pro' ), $capability, 'wpuf-profile-builder', array( $this, 'wpuf_profile_builder_page' ) );
    }
     ...
     * @return void
     */
    public function wpuf_profile_builder_page() {
    ...
    public function wpuf_user_listing_init( $atts ) {
        $atts = shortcode_atts(
            array(
                'role'          => 'all',
                'per_page'      => '6',
        ...
        if ( $user_id ) {
            ...
            $all_user = $this->get_all_user( $atts['role'] );
    ...
    public function user_listing_content( $all_user ) {
    ...
    public function pagination() {
        ...
        $page_links = paginate_links(
            array(
                ...
                'format'    => '',
                'prev_text' => __( '&laquo;', 'wpuf-pro' ),
                'next_text' => __( '&raquo;', 'wpuf-pro' ),
                'total'     => $num_of_pages,
                'current'   => $pagenum,
            )
        );
    ...
    public function table_header() {
        ...
        if ( isset( $user_meta['settings']['avatar'] ) && $user_meta['settings']['avatar'] === true ) {
            ...
            <th><?php esc_html_e( 'Avatar', 'wpuf-pro' ); ?></th>
    ...
    public function list_content( $all_user ) {
        foreach ( $all_user as $key => $val ) {
            $this->list_content_single( $val );
        }
    }

    public function list_content_single( $user ) {
        $unique_meta           = $this->unique_meta;
        $user_listing_template = wpuf_get_option( 'user_listing_template', 'user_directory', 'list' );
            ...
            <div class="user-box">
                <a href="<?php echo $this->get_user_link( $user->ID ); ?>" class="box-container">
                    ...
                    <div class="user-info">
                        ...
                        ?>
                        <span style="font-size: 13px;"><?php esc_html_e( 'View Profile', 'wpuf-pro' ); ?></span>
                    </div>
                </a>
            </div>
            <?php
    ...
    public function user_comments( $user_id, $post_type, $comment_count ) {
        $args = array(
            'user_id'     => $user_id,
            'order_by'    => 'post_date',
            'order'       => 'DESC',
        ...
        if ( $comments ) {
            ...
            esc_html_e( 'No comments found', 'wpuf-pro' );
        }
    }

    public function user_post( $user_id, $post_type, $post_count ) {
        $args = array(
            'author'         => $user_id,
            'post_type'      => $post_type,
    ...
    public function profile_nav_menus( $nav_urls ) {
        ...
        foreach ( $profile_fields['fields'] as $key => $field ) {
            ...
            switch ( $field['type'] ) {
                case 'meta':
                    ...
                    if ( is_array( $value ) ) {
                        $value = implode( ', ', $value );
                    } elseif ( ! empty( $userdata->$meta_key ) ) {
